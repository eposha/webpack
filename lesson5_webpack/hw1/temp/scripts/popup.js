...
import { check, generateNumbersRange } from './utilities.js'; //popup
...
const selectDay = event => {
  ...
  if (event.target.classList.value === 'table-sections__section') {
    ...
    startTime.value = "".concat(check(getTime - 1) + ':00');
    endTime.value = "".concat(check(getTime) + ':00');
    description.value = '';
    return;
  }
  ...
  if (event.target.classList.contains("add-event")) {
    ...
    startTime.value = "".concat(check(dateNow.getHours()) + ':00');
    endTime.value = "".concat(check(dateNow.getHours() + 1) + ':00');
    description.value = '';
  } else {
    ...
    endDate.value = startDate.value;
    startTime.value = "".concat(valueStratHour + ':' + valueStartMin);
    endTime.value = "".concat(valueEndHour + ':' + valueEndMin);
...
const TimeElem = () => {
  let resultTime = [];
  generateNumbersRange(0, 23).map(timeList => {
    let setTime = '';
    let setSec = 0;
    let cell = '00';

    for (let i = 0; i < 4; i++) {
      timeList < 10 ? setTime = "0".concat(timeList) : setTime = timeList;
      resultTime.push("<option \n            value=\"".concat(setTime, ":").concat(cell, "\"\n            data-block-number='").concat(timeList, "'>").concat(setTime, ":").concat(cell, "</option> "));
      setSec += 15;
      cell = setSec;
    }
