...
import { moveRedline } from './redline';
...
export const setCurrentMonth = () => {
  ...
  for (const arg of arrWithoutFirstArg) {
  ...
  const currentMonth = months[new Date(timeNow).getMonth()];
  ...
  if (findFirstDay === undefined) {
    ...
    generateNumbersRange(1, 7 - firstDate)
      .map((sectionNumber) => {
        ...
        const monthsNum = monthNum;
        const yearNum = nextYearForBlock;
        result.push(
          `
                <div 
